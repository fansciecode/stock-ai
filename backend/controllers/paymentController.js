                    ...
                    try {
                        ...
                        if (type === 'product') {
                            ...
                            if (payment.paymentInfo.eventId) {
                                ...
                                seller = await BusinessModel.findOneAndUpdate(
                                    { user: event.organizer },
                                    {
                                        $setOnInsert: {
                                            user: event.organizer,
                                            businessName: (await UserModel.findById(event.organizer))?.name || 'Business of ' + event.organizer,
                                            businessType: 'EVENT_ORGANIZER',
                                            registrationDetails: {
                                                registrationNumber: `AUTO-${event.organizer.toString().slice(-6)}-${Date.now()}`,
                                                registrationType: 'SOLE_PROPRIETORSHIP',
                                                registrationDate: new Date()
                                            },
                                            contactInfo: {
                                                email: (await UserModel.findById(event.organizer))?.email || `user${event.organizer}@example.com`
                                            },
                                            status: 'ACTIVE'
                                        }
                                    },
                                ...
                                seller = await BusinessModel.findOneAndUpdate(
                                    { user: firstProduct.seller },
                                    {
                                        $setOnInsert: {
                                            user: firstProduct.seller,
                                            businessName: (await UserModel.findById(firstProduct.seller))?.name || 'Business of ' + firstProduct.seller,
                                            businessType: 'RETAIL',
                                            registrationDetails: {
                                                registrationNumber: `AUTO-${firstProduct.seller.toString().slice(-6)}-${Date.now()}`,
                                                registrationType: 'SOLE_PROPRIETORSHIP',
                                                registrationDate: new Date()
                                            },
                                            contactInfo: {
                                                email: (await UserModel.findById(firstProduct.seller))?.email || `user${firstProduct.seller}@example.com`
                                            },
                                            status: 'ACTIVE'
                                        }
                                    },
