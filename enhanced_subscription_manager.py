...
class EnhancedSubscriptionManager:
    ...
    def can_user_select_plan(self, user_id: str, target_tier: str) -> Dict[str, Any]:
        ...
        if not subscription_state["has_subscription"]:
            return {
                "can_select": True,
                "action": "subscribe",
                "message": f"Subscribe to {target_tier.title()} plan"
            }
        ...
        if subscription_state["status"] in ["expired", "grace_period"]:
            return {
                "can_select": True,
                "action": "resubscribe",
                "message": f"Reactivate with {target_tier.title()} plan"
            }
    ...
    def stop_trading_for_expired_users(self):
        """Stop trading for users with expired subscriptions"""
        try:
            # Import here to avoid circular imports
            from fixed_continuous_trading_engine import FixedContinuousTradingEngine
            
            # Get all users with expired subscriptions who might be trading
            conn = sqlite3.connect('../../data/users.db')
            cursor = conn.cursor()
            ...
            if expired_users:
                # Get trading engine instance and stop trading for expired users
                engine = FixedContinuousTradingEngine()
                for user_id in expired_users:
                    # Convert user_id to email (assuming they match or we need to look up)
                    user_email = f"{user_id}@example.com"  # This needs proper lookup
                    if user_email in engine.active_sessions:
                        engine.stop_continuous_trading(user_email, "Subscription expired")
                        self.logger.info(f"Stopped trading for expired user: {user_id}")
            
        except Exception as e:
            self.logger.error(f"Error stopping trading for expired users: {e}")
