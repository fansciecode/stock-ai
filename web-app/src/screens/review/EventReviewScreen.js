...
const EventReviewScreen = () => {
  ...
  const fetchEventAndReviews = async () => {
    try {
      ...
      // Use a default eventId for demo if none is provided
      const id = eventId || 'event-123';
      ...
      setError(err.response?.data?.message || 'Failed to load event and reviews');
      setLoading(false);
    }
  };
  
  const checkUserAttendanceStatus = async () => {
    if (!user) return;
    
    try {
      const { data } = await axios.get(`/api/events/${eventId || 'event-123'}/attendance-status`);
      setUserHasAttended(data.hasAttended);
    } catch (err) {
      console.error('Failed to check attendance status:', err);
      // Default to true for demo purposes
      setUserHasAttended(true);
    }
  };
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `/api/events/${eventId || 'event-123'}/reviews`, 
        formData,
        {
          headers: {
  ...
  const getFilteredAndSortedReviews = () => {
    ...
    switch (reviewFilter) {
      ...
      case 'withPhotos':
        ...
        break;
      case 'verified':
        filteredReviews = filteredReviews.filter(review => review.verifiedAttendee);
        break;
      default:
  ...
  const demoReviews = [
    {
      _id: 'review-1',
      rating: 5,
      text: 'Best festival I\'ve been to this year! The performances were amazing and the organization was perfect.',
    ...
    },
    {
      _id: 'review-2',
      rating: 4,
      text: 'Great performances but the food options were limited. The sound quality was excellent though!',
      ...
      notHelpfulVotes: 1,
      verifiedAttendee: true,
      images: ['https://via.placeholder.com/100?text=Festival+Photo']
    },
    {
      _id: 'review-3',
      rating: 2,
      text: 'Too crowded and overpriced. Long lines for bathrooms and the headliner was 30 minutes late.',
    ...
    },
    {
      _id: 'review-4',
      rating: 5,
      text: 'Incredible atmosphere and the lineup was fantastic! Will definitely attend next year. The light show during the main act was breathtaking.',
  ...
  const displayEvent = event || demoEvent;
      ...
      <Box my={4}>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={9}>
              ...
              <Box display="flex" alignItems="center" mb={1}>
                <Rating value={displayEvent.rating} precision={0.5} readOnly />
                <Typography variant="body2" color="text.secondary" sx={{ ml: 1 }}>
                  {displayEvent.rating.toFixed(1)} ({displayEvent.numReviews} reviews)
                </Typography>
              </Box>
              
              <Box display="flex" alignItems="center" mb={1}>
                <Event color="action" sx={{ mr: 1 }} fontSize="small" />
                <Typography variant="body2">
                  {formatDate(displayEvent.date)} at {displayEvent.time}
                </Typography>
              </Box>
              ...
              <Typography variant="body1" color="primary" gutterBottom>
                ${displayEvent.ticketPrice?.toFixed(2)}
              </Typography>
              ...
              <Box mt={2}>
                <Button 
                  variant="contained" 
                  onClick={() => navigate(`/events/${displayEvent._id}`)}
                >
                  View Event
                </Button>
              </Box>
        ...
        <Grid container spacing={4}>
          <Grid item xs={12} md={4}>
            <Paper elevation={3} sx={{ p: 3 }}>
              ...
              <Box display="flex" alignItems="center" mb={2}>
                <Typography variant="h3" component="span" sx={{ mr: 1 }}>
                  {displayEvent.rating.toFixed(1)}
                </Typography>
                <Box>
                  <Rating value={displayEvent.rating} precision={0.5} readOnly />
                  <Typography variant="body2" color="text.secondary">
                    {displayEvent.numReviews} reviews
                  </Typography>
                </Box>
              </Box>
              ...
              {[5, 4, 3, 2, 1].map(rating => (
                <Box key={rating} display="flex" alignItems="center" mb={1}>
                  ...
                  >
                    <Box 
                      sx={{ 
                        width: `${(ratingCounts[rating] / displayEvent.numReviews) * 100}%`, 
            ...
            {(user && !userHasReviewed && userHasAttended) && (
            ...
            {(!userHasAttended && user && !userHasReviewed) && (
              <Paper elevation={3} sx={{ p: 3, mt: 3, bgcolor: 'info.light' }}>
                <Typography variant="body1" gutterBottom>
                  You need to attend this event before you can leave a review.
                </Typography>
                <Button 
                  variant="contained" 
                  color="primary"
            ...
            <Paper elevation={3} sx={{ p: 3 }}>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                <Typography variant="h6">
                  Attendee Reviews ({displayReviews.length})
                </Typography>
                ...
                <Box display="flex" alignItems="center">
                  <FormControl variant="outlined" size="small" sx={{ minWidth: 120, mr: 1 }}>
                    ...
                    >
                      ...
                      <MenuItem value="verified">Verified Attendees</MenuItem>
                    </Select>
                  </FormControl>
