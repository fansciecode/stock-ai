...
class ExternalEventService {
  constructor() {
    this.baseUrl = '/api/external-events';
  }
   ...
   */
  async getExternalEvents(params = {}) {
    try {
   ...
   */
  async getExternalEventDetails(source, eventId) {
    try {
      const response = await api.get(`${this.baseUrl}/${source}/${eventId}`);
      ...
      console.error('Failed to fetch external event details:', error);

      // Return mock data as fallback
      const mockEvent = this.getMockEventDetails(eventId);
      return {
        success: true,
        data: mockEvent
      };
  ...
  async registerForEvent(eventId, registrationData) {
    try {
      const response = await api.post(`${this.baseUrl}/${eventId}/register`, registrationData);
      ...
      return {
        ...
        data: {
          ...
          eventId: eventId,
          confirmationCode: `CONF-${Math.floor(10000 + Math.random() * 90000)}`,
  ...
  async searchEvents(query, filters = {}) {
    try {
      ...
      return await this.getExternalEvents(params);
    } catch (error) {
  ...
  async getPopularEvents(limit = 10) {
    try {
      ...
      if (response.success) {
        ...
        return this.getMockPopularEvents(limit);
      }
    } catch (error) {
  ...
  async getEventsByLocation(latitude, longitude, radius = 50) {
    try {
      ...
      if (response.success) {
        ...
        return this.getMockNearbyEvents();
      }
    } catch (error) {
  ...
  getMockEvents(params = {}) {
    const mockEvents = [
      {
        ...
        description: 'Annual music festival featuring top artists from around the world. Join us for an unforgettable experience with live performances, food trucks, and entertainment.',
        category: 'music',
        date: '2024-07-15',
        location: {
      ...
      },
      {
        ...
        description: 'Explore the latest in technology and innovation. Network with industry leaders and discover cutting-edge solutions.',
        category: 'tech',
        date: '2024-08-22',
        location: {
      ...
      },
      {
        ...
        description: 'Featuring works from contemporary artists. Discover new perspectives and artistic expressions.',
        category: 'art',
        date: '2024-09-10',
        location: {
      ...
      },
      {
        ...
        description: 'Taste dishes and wines from award-winning chefs and vineyards. A culinary adventure awaits.',
        category: 'food',
        date: '2024-07-30',
        location: {
          address: 'Waterfront Park, Chicago, IL',
          city: 'Chicago',
          state: 'IL',
          latitude: 41.878113,
          longitude: -87.629799
      ...
      },
      {
        id: 'ext5',
        title: 'Marathon 2024',
        description: 'Annual city marathon with routes for all skill levels. Challenge yourself and support a great cause.',
        category: 'sports',
        date: '2024-10-15',
        location: {
    ...
    if (params.location) {
      filteredEvents = filteredEvents.filter(event =>
        event.location.address.toLowerCase().includes(params.location.toLowerCase()) ||
        event.location.city.toLowerCase().includes(params.location.toLowerCase())
      );
    }
  ...
  getMockEventDetails(eventId) {
    const mockEvents = this.getMockEvents().events;
    const event = mockEvents.find(e => e.id === eventId);

    if (event) {
      return {
        ...event,
        longDescription: event.description + ' This is a detailed description with more information about the event, including what to expect, schedule, and additional details.',
        schedule: [
          { time: '09:00', activity: 'Registration & Check-in' },
          { time: '10:00', activity: 'Opening Ceremony' },
          { time: '11:00', activity: 'Main Event' },
          { time: '15:00', activity: 'Break & Networking' },
          { time: '16:00', activity: 'Closing Ceremony' }
        ],
        requirements: [
          'Valid photo ID required',
          'Minimum age 18 (or 21 for alcohol events)',
  ...
  getMockPopularEvents(limit) {
    const mockEvents = this.getMockEvents().events;
    return mockEvents.slice(0, limit);
  }

  getMockNearbyEvents() {
    return this.getMockEvents().events.slice(0, 3);
  }
}
